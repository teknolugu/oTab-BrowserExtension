<template>
  <div class="menu">
    <el-menu class="el-menu" mode="horizontal" text-color="#a0cfff" active-text-color="#409EFF" :default-active="activeMenu">
      <el-menu-item class="select-board-item">
        <el-select placeholder="Select Board" name="select-input" size="medium" :value="board.id" @change="activeBoardChange">
          ...
          <el-option v-for="board in $store.getters.boardList" :key="board.id" :label="board.title" class="board-item-name" :value="board.id">
            ...
            <i @click.stop="staredBoard(board.id)" :class="[board.id === $store.state.starBoard ? 'el-icon-star-on stared' : 'el-icon-star-off not']" class="star-board-menu"></i>
          </el-option>
        </el-select>
        <el-button icon="el-icon-s-home" class="allBoards-button" size="small" @click="emitBus('home')" title="Home"></el-button>
        <el-button icon="el-icon-menu" class="allBoards-button" size="small" @click="emitBus('boardManager')" title="Boards"></el-button>
      </el-menu-item>
      ...
      <el-menu-item class="right" @click="emitBus('settings', 'general')">
...
import starBoard from '../../mixins/star-board';
import SelectTag from './select-tag';
import Bus from '../utils/bus';
export default {
  mixins: [newBoard, starBoard],
